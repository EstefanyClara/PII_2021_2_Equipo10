            ...
            }if(DataUserContainer.Instance.UserDataHistory.Keys.Contains(message.Id) && DataUserContainer.Instance.UserDataHistory[message.Id][0][0].Equals("/buscar"))
            {
                ...
                if(userData[0].Equals("Nombre"))
                {
                    if(AppLogic.Instance.SearchOfferByKeywords(message.Text) != null)
                    {
                        ...
                        StringBuilder sb = new StringBuilder($"Ofertas de {message.Text} disponibles");
                        foreach (IOffer item in list)
                        {
                            sb.Append($"{indice} - {item.Product.Classification} - cantidad: {item.Product.Quantity} - precio unitario: {item.Product.Price} - ubicación: {item.Product.Ubication} - fecha de publicación: {item.DatePublished}\n");
                        }
                        response = sb.ToString();
                        return true;
                    }
                }
                if(userData[0].Equals("Ubicacion"))
                {
                    if(AppLogic.Instance.SearchOfferByKeywords(message.Text) != null)
                    {
                        int indice = 1;
                        List<IOffer> list = AppLogic.Instance.SearchOfferByUbication(message.Text);
                        StringBuilder sb = new StringBuilder($"Ofertas en {message.Text} disponibles");
                        foreach (IOffer item in list)
                        {
                            sb.Append($"{indice} - {item.Product.Classification} - cantidad: {item.Product.Quantity} - precio unitario: {item.Product.Price} - ubicación: {item.Product.Ubication} - fecha de publicación: {item.DatePublished}\n");
                        }
                ...
                if(userData[0].Equals("Clasificacion"))
                {
                    if(AppLogic.Instance.SearchOfferByKeywords(message.Text) != null)
                    {
                        int indice = 1;
                        List<IOffer> list = AppLogic.Instance.SearchOfferByType(message.Text);
                        StringBuilder sb = new StringBuilder($"Ofertas de tipo {message.Text} disponibles");
                        foreach (IOffer item in list)
                        {
                            sb.Append($"{indice} - {item.Product.Classification.Category} - cantidad: {item.Product.Quantity} - precio unitario: {item.Product.Price} - ubicación: {item.Product.Ubication} - fecha de publicación: {item.DatePublished}\n");
                        }
                        response = sb.ToString();
                        return true;
                    }
